## Alinhando a Terminologia DevOps, MLOps, DataOps, AIOps e LLMOps

**DevOps** é uma abordagem cultural e técnica que busca melhorar a colaboração entre equipes de desenvolvimento e operações. O principal objetivo do DevOps é acelerar a entrega de software, ao mesmo tempo que melhora a qualidade e a estabilidade das implantações. Para alcançar isso, o DevOps enfatiza a automação, a integração contínua, a entrega contínua e o feedback constante entre as equipes. Além de técnicas e práticas, o **DevOps** destaca a importância de quebrar silos organizacionais para promover uma mentalidade de colaboração em toda a empresa.

**MLOps**, ou DevOps para Machine Learning, é uma abordagem que traz os princípios do DevOps para o mundo do Machine Learning. A ideia é facilitar a colaboração entre profissionaisde dados e equipes de operações para permitir a construção, teste e implantação mais rápida e confiável de modelos de Machine Learning em produção. MLOps aborda desafios específicos no ciclo de vida de modelos ML, como gerenciamento de versão de modelo, monitoramento de desempenho de modelo e re-treinamento.
...
- **Colaboração**: Facilitar a colaboração entre cientistas de dados, engenheiros de Machine Learning e equipes de operações, garantindo que todos estejam alinhados em relação aos objetivos e práticas.

- **Governança e Conformidade**: Assegurar que os modelos atendam aos padrões regulatórios e éticos, e que exista transparência em suas decisões.

- **Versionamento**: Manter um registro das diferentes versões de modelos e conjuntos de dados, permitindo rollbacks e comparações quando necessário.

- **Validação e Teste**: Implementar testes rigorosos para modelos, garantindo que eles atendam a padrões de qualidade antes de serem implantados em produção

- **Integração Contínua e Entrega Contínua (CI/CD)**: Assim como no DevOps, MLOps busca a integração e entrega contínuas, mas adaptadas às especificidades dos modelos de Machine Learning.
...
MLOps Engineering at Scale. Autor: Carl Osipov. Editora: Manning
